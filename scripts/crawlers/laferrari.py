"""
La Ferrari Macau Crawler
Domain: laferrari.com.mo (suspected defunct) / laferrarimacau.com
Tier 3 (formerly disabled) — weekly crawl
La Ferrari Macau was an Italian restaurant/bar. Site status uncertain.
Crawler tries multiple domain candidates and degrades gracefully.
"""
...
class LaFerrariCrawler(BaseCrawler):
    name = "laferrari.com.mo"
    base_url = "https://www.laferrari.com.mo"
    rate_limit = 4.0
    frequency = "weekly"

    # Try multiple domain candidates — original .mo domain may be defunct
    SEED_URLS = [
        "https://www.laferrari.com.mo/events",
        "https://www.laferrari.com.mo/",
        "https://www.laferrarimacau.com/events",
        "https://www.laferrarimacau.com/",
    ]

    VENUE_NAME = "La Ferrari"
    VENUE_ADDRESS = "Macau"
    VENUE_COORDINATES = {"lat": 22.1937, "lng": 113.5418}  # placeholder
    ...
    async def fetch_list_urls(self) -> List[str]:
        ...
        for seed in self.SEED_URLS:
            try:
                ...
                # Quick sanity check — if page looks like a parking page, skip
                if self._is_parked_page(html):
                    self.logger.info("La Ferrari: %s appears to be a parked/dead domain", seed)
                    continue
                ...
                self.logger.debug("La Ferrari seed %s failed: %s", seed, e)
                continue

        if not found:
            self.logger.warning(
                "La Ferrari: no events found — domain may be defunct. "
                "Consider disabling this crawler in CRAWL_REGISTRY."
            )
        return found

    def _is_parked_page(self, html: str) -> bool:
    ...
    def _extract_event_links(self, html: str, base: str) -> List[str]:
        ...
        links: list[str] = []
        allowed_domains = {"laferrari.com.mo", "laferrarimacau.com"}
        ...
        for a in soup.find_all("a", href=True):
            ...
            if (
                ...
                and any(
                    seg in full
                    for seg in ["/event", "/news", "/promotion", "/special", "/offer"]
                )
    ...
    async def parse_event(self, url: str, html: str) -> Optional[CrawledEvent]:
        ...
        event = CrawledEvent(
            ...
            tags=["italian", "restaurant", "bar", "la ferrari", "macau"],
            language="en",
    ...
    def _extract_title(self, soup) -> str:
        for sel in ["h1.event-title", "h1", ".entry-title", "title"]:
            el = soup.select_one(sel)
            if el:
                t = el.get_text(" ", strip=True)
                if t and t.lower() not in {"la ferrari", "home", "events"}:
                    return t
        return ""

    def _extract_description(self, soup) -> str:
        for sel in [".event-content", ".entry-content", "article p", "main p"]:
            el = soup.select_one(sel)
            if el:
    ...
    def _apply_venue_defaults(self, event: CrawledEvent) -> None:
        ...
        if not event.categories:
            event.categories = ["dining", "nightlife"]
